...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const App = {
        ...
        bindEvents() {
            ...
            this.elements.voiceBtn.addEventListener('click', this.toggleVoice.bind(this));
        },
        ...
        addMessageToChat(message, sender) {
            ...
            // Handle structured tool output if it exists (don't show JSON to user)
            let displayMessage = message;
            try {
                if (message.includes('```json')) {
                    const jsonPart = message.match(/```json\n([\s\S]*?)\n```/);
                    if (jsonPart) {
                        displayMessage = message.replace(jsonPart[0], '').trim();
                        if (!displayMessage) displayMessage = "Executing command...";
                    }
                }
            } catch (e) {}
        ...
        async getGeminiResponse(prompt) {
            ...
            try {
                // System Instruction to make it an agent
                const systemPrompt = `You are the ZenDesk Pro Automation Agent. 
                You can help the user manage their dashboard. 
                When the user asks to do something, you MUST respond with a JSON block at the end of your text using this format:
                \`\`\`json
                {
                  "actions": [
                    {"type": "ADD_TASK", "text": "task description", "priority": "low|medium|high"},
                    {"type": "DELETE_TASK", "id": 123},
                    {"type": "ADD_NOTE", "text": "note content"},
                    {"type": "START_TIMER", "seconds": 1500},
                    {"type": "PAUSE_TIMER"},
                    {"type": "RESET_TIMER"}
                  ]
                }
                ...
                const historyWithSystem = [
                    ...
                    { role: 'model', parts: [{ text: "Understood. I am now the ZenDesk Pro Automation Agent. I will help you manage your dashboard using the specified JSON format for actions." }] },
                ...
                this.processActions(botResponse);
                this.speak(botResponse);

            } catch (error) {
                console.error("Error fetching Gemini response:", error);
                const thinkingMessage = this.elements.chatbotMessages.lastChild;
                if (thinkingMessage && (thinkingMessage.textContent === "Thinking..." || thinkingMessage.textContent === "Executing command...")) {
                    this.elements.chatbotMessages.removeChild(thinkingMessage);
                }
            ...
            try {
                ...
                if (data.actions && Array.isArray(data.actions)) {
                    data.actions.forEach(action => {
                        ...
                        switch (action.type) {
                            case 'ADD_TASK':
                                this.elements.taskInput.value = action.text;
                                this.elements.taskPriority.value = action.priority || 'medium';
                                this.addTask();
                                break;
                            case 'DELETE_TASK':
                                ...
                                break;
                            case 'ADD_NOTE':
                                this.elements.noteInput.value = action.text;
                                this.addNote();
                                break;
                            case 'START_TIMER':
                                this.state.timerSeconds = action.seconds || this.state.timerSeconds;
                                this.startTimer();
                                break;
                            case 'PAUSE_TIMER':
                                this.pauseTimer();
        ...
        speak(text) {
            // Clean text of JSON blocks before speaking
